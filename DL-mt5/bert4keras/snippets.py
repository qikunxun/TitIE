...
class AutoRegressiveDecoder(object):
    ...
    def _is_finished(self, t, s):
        ...
        length_penalty = tf.to_float(self.maxlen)
        ...
        cond = tf.logical_and(tf.less(t, self.maxlen), tf.logical_not(bound_is_met))  # 每条最差>最好，返回0
    ...
    def _beam_search_step(self, time, state):
        ...
        print(flat_seqs)
        logits = self.predict(self.memory, flat_seqs, None)
        ...
        flags = tf.equal(symbol_indices, self.end_id)
        ...
        pad_seqs = tf.fill([self.batch_size, self.beam_size, 1], tf.constant(self.start_id, tf.int32))
    ...
    def beam_search(self, inputs, topk, states=None, temperature=1, min_ends=1):
        ...
        #             return output_ids[best_one], output_scores[best_one]  # 直接输出
        #         else:  # 否则，只保留未完成部分
        ...
        memory = tf.constant(inputs, dtype=tf.float32)
        self.beam_size = topk
        ...
        # self.features['title_idx'] = tf_utils.merge_first_two_dims(
        #     tf_utils.tile_to_beam_size(title_idx, self.beam_size))
        self.batch_size = tf_utils.infer_shape(inputs)[0]
        ...
        init_seqs = tf.fill([self.batch_size, self.beam_size, 1],
                            self.start_id, )  # [self.batch_size, self.beam_size, 1] 初始序列填充的是start_token_idx
        ...
        return self.final_seqs, self.final_scores
