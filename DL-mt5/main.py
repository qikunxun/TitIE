...
valid_data = load_valid_data(data_file_valid)
test_data = load_valid_data(data_file_valid)
...
class data_generator(DataGenerator):
    ...
    def __iter__(self, random=False):
        batch_c_token_ids, batch_t_token_ids, batch_reward = [], [], []
        for is_end, (id, query, title, reward) in self.sample(random):
            ...
            if len(batch_c_token_ids) == self.batch_size or is_end:
                ...
                yield [batch_c_token_ids, batch_t_token_ids, batch_reward], None
                batch_c_token_ids, batch_t_token_ids, batch_reward = [], [], []
...
class CrossEntropy(Loss):
    ...
    def compute_loss(self, inputs, mask=None):
        y_true, y_pred, reward = inputs
        y_true = y_true[:, 1:]  # 目标token_ids
        y_mask = K.cast(mask[1], K.floatx())[:, :-1]  # 解码器自带mask
        y_pred = y_pred[:, :-1]  # 预测序列，错开一位
        loss = K.sparse_categorical_crossentropy(y_true, y_pred)
        loss = K.sum(loss * y_mask, axis=-1) / K.sum(y_mask, axis=-1)
        loss = loss * reward
        loss = K.mean(loss)
        return loss
...
with tf.variable_scope('t5'):
    ...
    decoder = t5.decoder
    model_t5 = t5.model
    reward = Input(shape=(None, ), dtype='float32', name='reward')
    model_t5.inputs.append(reward)
    model_t5.summary()
    outputs = BeamSearchLayer(decoder, max_query_length, 0, tokenizer._token_end_id, beam_size=1)([encoder.outputs, encoder.inputs])
    model_search = Model(model_t5.inputs[:-1], outputs)

    output = CrossEntropy(1)([model_t5.inputs[1], model_t5.outputs[0], model_t5.inputs[-1]])
...
class Autoquery(AutoRegressiveDecoder):
    ...
    def generate(self, text, topk=1):
        ...
        output_ids, score = self.random_sample([c_encoded], 1)  # 基于beam search
...
if __name__ == '__main__':
    for i in range(max_epochs):
        ...
        score = calculate_score(queries, pred_queries)
        tf.logging.info('Score: {}'.format(str(score)))
        ...
        for episode in range(episodes):
            ...
            for start in tqdm(range(0, num_expamles_rand, batch_size)):
                ...
                for k in range(len(batch)):
                    if batch_pred[k] == 0:
                        batch_new.append((batch[k][0], batch[k][1], batch[k][2], 1.))
                if len(batch_new) == 0:
                    rewards.append(0)
                    for k in range(len(batch)):
            ...
            train_generator_pn = data_generator_pn(train_data_pn, batch_size)
            model_pn.fit(
        ...
        reward_un = prediction_un
        prediction_un = prediction_un.argmax(axis=-1)
        remove_index = set()
        train_data_u_new = []
        for j, item in enumerate(train_data_u_sample):
            if prediction_un[j] == 0:
                id = item[0]
                query = item[1]
                title = item[2]
                train_data.append((id, query, title, reward_un[j, 0]))
                remove_index.add(id)
            else:
                id = item[0]
                title = item[2]
